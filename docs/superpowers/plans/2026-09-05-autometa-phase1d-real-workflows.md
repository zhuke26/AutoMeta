...
- [x] Write repository tests proving newest-first job listing, stage filtering, and Review isolation.
- [x] Run `pytest tests/api/test_workflow_jobs.py tests/services/test_workflow_coordinator.py -q` and verify failures because the repository and coordinator do not exist.
- [x] Implement these concrete coordinator methods:
...
- [x] Add `JobRepository.list_for_review(review_id, stage=None, limit=20)` and the Review-scoped GET endpoint.
- [x] On submission create a `StageRun(status="queued")`; on job transitions update it to `running`, `succeeded`, `failed`, `interrupted`, or `cancelled` without storing secrets.
- [x] Add API tests for 404, 409, filtering, terminal results, and interrupted jobs visible after app restart.
- [x] Run the focused tests and the complete Python suite.
- [x] Commit as `feat: add Review workflow coordination`.
...
- [x] Write failing hook/component tests for debounce, approval, stale rejection, SSE replay, browser disconnect, polling fallback, interrupted state, and server errors.
- [x] Implement exact artifact request bodies already accepted by the API:
...
- [x] Implement `JobProgressPanel` so queued/running/interrupted/failed/succeeded are text-labelled; never infer success from a closed SSE stream.
- [x] Ensure no browser storage or request includes API credentials.
- [x] Run focused tests, the full frontend suite, typecheck, and build.
- [x] Commit as `feat: add durable workflow client infrastructure`.
...
- [x] Write failing API tests with a fake `ProtocolAgent` proving immediate job return, continued execution without SSE, saved draft, safe failure, and 409 on concurrent protocol jobs.
- [x] Implement the Review-scoped protocol job and save its output through `ArtifactService.save_draft`.
- [x] Write failing React tests for manual PICO entry, draft generation, restored progress, field editing, 600 ms autosave, and Approve enabling the Search route.
- [x] Replace the Setup placeholder with the research-question form and four explicit P/I/C/O fields. For independent entry modes, allow direct PICO entry without model generation.
- [x] Require nonempty P, I, and O before approval; C may explicitly say `No comparator specified`.
- [x] Run backend/frontend focused tests and full typecheck/build.
- [x] Commit as `feat: add persisted PICO Review setup`.
...
- [x] Write failing API tests using a fake SearchAgent for upstream approval enforcement, empty-query rejection, job events, records persistence, and PubMed errors.
- [x] Implement the two job operations with exact input artifact IDs recorded in `StageRun`.
- [x] Write failing React tests for query generation, raw-query editing/autosave, approval gate, retrieval settings, job reconnection, record search/sort/pagination, and JSON/CSV/RIS exports.
- [x] Implement the real Search page; disabled controls remain disabled until their required artifact is approved.
- [x] Approve Records explicitly before Screening can consume them.
- [x] Run focused and complete suites, typecheck, and build.
- [x] Commit as `feat: migrate the Search workflow`.
...
- [x] Write failing API tests for JSON/CSV validation, approved-input enforcement, dimension evidence preservation, durable progress, error stop, and draft output.
- [x] Implement import and job routes without using the legacy connection-owned streaming endpoint.
- [x] Write failing React tests for independent import, P/I/C/O columns, evidence text, score/confidence sorting, filters, Top N, bulk/manual selection, uncertain retention, autosave, and approval.
- [x] Implement the Screening page with real persisted decisions and selected IDs; do not imply automatic exclusion.
- [x] Run focused and complete suites, typecheck, and build.
- [x] Commit as `feat: migrate the Screening workflow`.
...
- [x] Write migration/storage tests for PDF compatibility, CSV separation, ownership, invalid signatures, duplicate files, and Review deletion.
- [x] Write failing workflow tests for acknowledgement gating, no upstream-selected-studies requirement in extraction-only mode, durable execution, citation preservation, and failures.
- [x] Write failing React tests for first-run disclosure, manual PDF upload, file restoration, field definitions, job progress, editable results, researcher-edit markers, selected meta-analysis rows, CSV/JSON export, and approval.
- [x] Implement the disclosure acknowledgement through a nonsecret local setting API; never place file text or credentials in job events.
- [x] Replace the Extraction placeholder and run all focused/full gates.
- [x] Commit as `feat: migrate the Extraction workflow`.
...
- [x] Write failing API tests for Review file ownership, invalid CSV, plan persistence, approval gate, strict validation failure, generated code/result persistence, and no silent method substitution.
- [x] Write failing React tests for CSV upload, summaries, editable structured plan fields, assumptions/warnings, approval, failed-run errors, pooled effect, confidence interval, Q/I²/tau², study effects, weights, logs, code, and exports.
- [x] Implement the Meta-analysis page with no fabricated forest plot; render only outputs returned by the validated engine.
- [x] Run focused/full suites, typecheck, build, and deterministic calculation fixtures already supported by the current runner.
- [x] Commit as `feat: migrate the Meta-analysis workflow`.
...
- [x] Write a backend integration test that uses fake agents to run all stages, edits an approved upstream artifact, and proves downstream artifacts become stale and cannot be consumed.
- [x] Write a frontend integration test that resumes an interrupted job, approves each artifact, navigates downstream, and never exposes an enabled dead control.
- [x] Replace every pending route with the real page and remove placeholder copy from production assets.
- [x] Search source and compiled output for `Workflow migration pending`, `Stage unavailable`, `Load Example`, benchmark identifiers, and legacy product naming; require no production hits.
- [x] Run complete frontend/backend suites, typecheck, audit, and build.
- [x] Commit as `feat: complete the four-stage React workflow`.
...
- [x] Document stage prerequisites, approval semantics, browser-close behavior, interrupted retry, PDF disclosure, and manual file support.
- [x] Run `npm ci`, frontend tests, typecheck, build, and confirm a second clean build produces no `autometa/static` diff.
...
- `npm ci && npm test -- --run && npm run typecheck && npm run build`: 16
  Vitest files and 56 tests passed; TypeScript and Vite completed successfully.
- A second `npm run build` followed by
  `git diff --exit-code -- autometa/static` produced no compiled-asset diff.
- `.venv/bin/python -m pytest tests -q`: 101 tests passed.
- `.venv/bin/ruff check .`: passed with the explicit project lint policy in
  `pyproject.toml`.
- `python -m build`: sdist and wheel built; wheel inspection confirmed the API,
  CLI, and hashed React assets and rejected credential, manuscript, benchmark,
  and local-data paths.
- A clean Python 3.12 virtual environment installed the wheel with dependencies,
  imported `autometa.api.main:app`, and ran `python -m autometa --help`.
- A temporary-data Uvicorn runtime returned a healthy `/api/v1/health` response.
  Browser verification exercised Guided setup/PICO approval and all four
  independent entry modes at 1024, 1280, 1440, and 1920 px; Library persistence,
  exact-name deletion gating, and redacted System Status were also checked.
- Browser console inspection returned no warnings or errors. Runtime assets were
  local, and source/build scans found no remote font/CDN, browser credential
  storage, benchmark/manuscript identifiers, or legacy product naming.
